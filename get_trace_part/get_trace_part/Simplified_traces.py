# This program is used to simplify each the split trace file
...
for eachFile in sourceFileList: #traserve the split files
    ...
    for line in currentFile:
        ...
        p_re=re.compile('\d\s.*?\s\d+\.\d+|\d\s(?!send)(?!allreduce).*?\s\d+\s\d+'); # use regular expression to extract useful information
