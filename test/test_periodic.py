...
def test_cml_oneshot(test_root_dir, issue_all_calls_at_start, time_unit):
...
def test_start0(test_root_dir, time_unit, is_infinite, is_probe):
    platform = 'small_platform_replay_usage'
    workload = 'test_delays'
    func_name = inspect.currentframe().f_code.co_name.replace('test_', '', 1)
    instance_name = f'{MOD_NAME}-{func_name}-{time_unit}-' + str(int(is_infinite)) + '-' + str(int(is_probe))

    edc_init_args = {
        'is_infinite': is_infinite,
        'time_unit': time_unit,
        'calls': [
            {'init': 0, 'period': 10, 'nb': 1093, 'is_probe': is_probe},
            {'init': 0, 'period': 100, 'nb': 179, 'is_probe': is_probe},
            {'init': 0, 'period': 1000, 'nb': 13, 'is_probe': is_probe},
            {'init': 0, 'period': 10000, 'nb': 3, 'is_probe': is_probe},
        ],
    }

    batargs = ["--energy-host"]
    batcmd, outdir, _ = prepare_instance(instance_name, test_root_dir, platform, 'periodic', workload, edc_init_content=json.dumps(edc_init_args, allow_nan=False, sort_keys=True), batsim_extra_args=batargs)
    p = run_batsim(batcmd, outdir)
    assert p.returncode == 0

def test_rand(test_root_dir, seed, time_unit, is_infinite):
    platform = 'small_platform_replay_usage'
    workload = 'test_delays'
    func_name = inspect.currentframe().f_code.co_name.replace('test_', '', 1)
    ...
    for _ in range(nb_periodic_calls):
        calls.append({
            ...
            'is_probe': bool(random.randint(0,1))
        })
