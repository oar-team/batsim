 ...
 *
 * @param[in] init_data The initialization data sent to your decision component. This is retrieved from Batsim's command-line arguments.
 * @param[in] init_size The size of the initialization data (in bytes). This is retrieved from Batsim's command-line arguments.
 * @param[out] flags These flags enable your EDC to decide how to communicate with Batsim.
 *                   Currently, this is only used to set which data format should be used (flatbuffers's binary format or flatbuffer's JSON format).
 * @param[out] reply_data The reply from your EDC. A pointer to a batprotocol Message allocated by your EDC, which should be serialized consistently with flags.
 * @param[out] reply_size The size of your reply (in bytes).
