...
JsonProtocolWriter::JsonProtocolWriter(BatsimContext *context) : _context(context), _alloc(_doc.GetAllocator())
{
  _doc.SetObject();
}
...
JsonProtocolReader::JsonProtocolReader(BatsimContext *context) : context(context)
{
  ...
  _type_to_handler_map["ADD_PROBE"] = &JsonProtocolReader::handle_add_probe;
}
                                            ...
                                            const Value &data_object)
{
  ...
  try
  {
    message->allocation->machine_ids = IntervalSet::from_string_hyphen(alloc, " ", "-");
  }
  ...
  if (data_object.HasMember("additional_io_job"))
  {
    ...
    try
    {
      message->allocation->io_allocation = IntervalSet::from_string_hyphen(alloc, " ", "-");
    }
    catch (const std::exception &e)
    {
      throw std::runtime_error(std::string("Invalid JSON message: bad IO allocation: ") + e.what());
    }
  }
  else
  {
                                                 ...
                                                 const Value &data_object)
{
  ...
  if (allowed_states.count(job_state) != 1)
  {
    xbt_assert(false, "Invalid JSON message: in event %d (CHANGE_JOB_STATE): "
                      "['data']['job_state'] must be one of: {%s}",
               event_number,
...
void JsonProtocolReader::handle_add_probe(int event_number,
                                          double timestamp,
                                          const Value &data_object)
{
  (void)event_number; // Avoids a warning if assertions are ignored
