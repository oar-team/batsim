...
JobIdentifier::JobIdentifier(const string &job_id_str)
{
    ...
    job_name = job_identifier_parts[1];
}
...
Job *Jobs::operator[](JobIdentifier job_name)
{
    auto it = _jobs.find(job_name);
    xbt_assert(it != _jobs.end(), "Cannot get job %s: it does not exist", job_name);
    return it->second;
}

const Job *Jobs::operator[](int job_name) const
{
    auto it = _jobs.find(job_name);
    xbt_assert(it != _jobs.end(), "Cannot get job %s: it does not exist", job_name);
    return it->second;
...
Job *Jobs::at(int job_name)
{
    return operator[](job_name);
}

const Job *Jobs::at(int job_name) const
{
    return operator[](job_name);
}
...
bool Jobs::exists(int job_name) const
{
    auto it = _jobs.find(job_name);
    return it != _jobs.end();
}
                     ...
                     const std::string & error_prefix)
{
    ...
    xbt_assert(json_desc["id"].IsString());
    job->id = JobIdentifier(job_desc["id"].GetString());
    ...
    XBT_DEBUG("Job '%s' Loaded", j->id.to_string().c_str(), j->workload->name.c_str());
    return j;
}
