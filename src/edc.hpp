...
struct ExternalLibrary
{
    ...
    uint8_t (*init)(const uint8_t *, uint32_t, uint32_t *, uint8_t **, uint32_t *) = nullptr; //!< A function pointer to the batsim_edc_init symbol in the loaded library.
...
public:
    ...
    void init(const uint8_t *data, uint32_t data_size, uint32_t * serialization_flag, uint8_t ** hello_buffer, uint32_t * hello_buffer_size);
