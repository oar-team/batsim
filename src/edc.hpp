...
struct ExternalLibrary
{
    ...
    uint8_t (*init)(const uint8_t *, uint8_t, uint8_t) = nullptr; //!< A function pointer to the batsim_edc_init symbol in the loaded library.
    uint8_t (*deinit)() = nullptr; //!< A function pointer to the batsim_edc_deinit symbol in the loaded library.
    uint8_t (*take_decisions)(const uint8_t*, uint8_t**) = nullptr; //!< A function pointer to the batsim_edc_take_decisions symbol in the loaded library.

    ExternalLibrary(const std::string & lib_path);
    ~ExternalLibrary();
};
