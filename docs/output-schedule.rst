...
Aggregated information about the schedule is exported as *prefix* + ``schedule.json`` (the prefix is `out/` by default, see :ref:`cli`).
...
- ``nb_jobs_rejected``: The number of rejected jobs in the simulation.
...
- ``time_unavailable``: Total time of all machines spent in unavailable state.
