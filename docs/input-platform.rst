...
The platform file is the only mandatory input of a simulation with Batsim.
...
Energy model
------------
SimGrid allows each host to have a set of power states (also known as pstates).
Each state defines several properties:

- The computation speed (number of floating-point operations per second).
- The electrical power consumption (in watts). This is usually defined with several values.
...
In Batsim, asking to change the pstate of some hosts is done using the ``ChangeHostsPStateEvent`` of the protocol, see :ref:`protocol` for more details.
...
Batsim users (= External Decision Components) can only switch into computation or sleep power states. Switching from one sleep power state to another is forbidden. Switching from one computation power state to another is instantaneous.
Asking to switch from a computation power state to a sleep state (or the reverse) is done using the ``TurnOnOffHostsEvent`` of the protocol, see :ref:`protocol` for more details.
